...
const NotesView: React.FC<NotesViewProps> = ({ notes, onAddNote, onUpdateNote, onDeleteNote }) => {
    ...
    const handleAdd = () => {
        if (newTitle.trim() || newContent.trim()) {
            onAddNote(newTitle.trim() || 'Senza titolo', newContent.trim());
            setNewTitle('');
            setNewContent('');
            setIsAdding(false);
        }
    ...
    const handleUpdate = () => {
        if (editingNote && (newTitle.trim() || newContent.trim())) {
            onUpdateNote(editingNote.id, newTitle.trim() || 'Senza titolo', newContent.trim());
            setEditingNote(null);
            setNewTitle('');
            setNewContent('');
        }
        ...
        <div className="space-y-8">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Salva i tuoi pensieri e appunti importanti.</p>
                </div>
                    ...
                    >
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="text-sm font-black uppercase tracking-widest text-brand-600 dark:text-brand-400">
                                {isAdding ? 'Crea Nuova Nota' : 'Modifica Nota'}
                            </h3>
                    ...
                    <div className="space-y-6">
                        ...
                        {filteredNotes.length === 0 && (
                            <div className="text-center py-20 px-4 glass-card rounded-[2.5rem] border-dashed border-2 border-slate-200 dark:border-slate-800">
                                ...
                                <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">Nessuna nota trovata</h3>
                                <p className="text-slate-500 dark:text-slate-400 max-w-xs mx-auto font-medium">
                                    {searchTerm ? 'Prova a cercare qualcos\'altro.' : 'Inizia a scrivere i tuoi pensieri cliccando su "Nuova Nota".'}
                                </p>
                            </div>
                        )}
                    </div>
